...
class Aiogram3BootstrapTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_startup_sets_db_commands_webhook_verifies_url_and_registers_jobs_once(self):
        ...
        self.assertEqual(len(fake_scheduler.jobs), 12)
        self.assertTrue(hasattr(self.main, "scheduled_apply_reserved_gifts"))
        self.assertTrue(hasattr(self.main, "scheduled_process_bookable_class_deadlines"))
    ...
    async def test_miniapp_users_route_is_bearer_protected_and_bounded(self):
        app = self.main.create_app()
        handler = self.route_handler(app, "GET", "/api/admin/users")
        session = SimpleNamespace(telegram_id=1, session_id="session-ref")
        ...
        request = FakeMiniAppRequest(
            ...
            query={"limit": "50", "status": "active", "q": "admin"},
        )
        missing = await self.main.miniapp_admin_auth_middleware(
            FakeMiniAppRequest(app, path="/api/admin/users"), handler
        )
        with patch.object(self.main, "load_miniapp_admin_session", return_value=session), \
             patch.object(self.main, "list_admin_users", return_value=result) as list_users, \
             patch.object(
                 self.main.stripe.Subscription,
                 "retrieve",
                 side_effect=AssertionError("Stripe must not be called"),
    ...
    async def test_miniapp_users_rejects_invalid_limit_and_status(self):
        app = self.main.create_app()
        handler = self.route_handler(app, "GET", "/api/admin/users")
        session = SimpleNamespace(telegram_id=1, session_id="session-ref")
        for query in ({"limit": "51"}, {"status": "dangerous"}):
            request = FakeMiniAppRequest(
                app, "Bearer token", path="/api/admin/users", query=query
            )
    ...
    async def test_real_member_routes_use_separate_bearer_auth_and_rollout_gate(self):
        ...
        member_routes = (
            ...
            ("POST", "/api/member/classes/{class_id}/book"),
    ...
    async def test_miniapp_static_routes_are_registered_with_secure_content(self):
        ...
        self.assertIn('data-nav="analytics"', index)
        self.assertIn('data-nav="notifications"', index)
        self.assertIn('data-nav="settings"', index)
        self.assertIsNotNone(self.route_handler(app, "GET", "/api/admin/analytics"))
        ...
        self.assertIn('id="fullscreen-message"', index)
        self.assertIn("webApp.requestFullscreen", javascript)
        self.assertNotIn("window.innerWidth < 1024 || Boolean(webApp.isFullscreen)", javascript)
        self.assertIn('new Set(["android", "android_x", "ios"])', javascript)
        self.assertIn('webApp.onEvent("fullscreenChanged"', javascript)
        self.assertIn('webApp.onEvent("fullscreenFailed"', javascript)
        self.assertIn("Полноэкранный режим недоступен в этой версии Telegram.", javascript)
        self.assertIn("adminModeConfirmed = true", javascript)
        self.assertIn("MINIAPP_VIEWPORT_DIAGNOSTIC", javascript)
        for panel_id in (
            "dashboard-content-list", "dashboard-users-list",
            "dashboard-schedule-list", "dashboard-gifts-list",
            "dashboard-failed-list", "dashboard-open-system",
        ):
            self.assertIn(f'id="{panel_id}"', index)
        ...
        self.assertIn("return loadDashboard().then(() => {", javascript)
        ...
        self.assertIn("width: min(100%, 1480px)", stylesheet)
        self.assertIn("grid-template-columns: minmax(0, 1fr) minmax(360px, 410px)", stylesheet)
        self.assertIn("#users-list .user-card > button", stylesheet)
        self.assertIn("background: var(--soul-surface) !important", stylesheet)
        self.assertIn("@media (max-width: 1023px)", stylesheet)
        self.assertIn("grid-template-columns: repeat(4, minmax(0, 1fr))", stylesheet)
        self.assertIn("Задачи планировщика", index)
        self.assertIn("Не доставлено окончательно", index)
        self.assertIn("--soul-bg", stylesheet)
        self.assertIn("--soul-shadow", stylesheet)
        self.assertIn("body.member-preview-mode .member-grid.member-content-grid", stylesheet)
        ...
        self.assertIn("Режим администратора", index)
        self.assertIn("Открыть в админке", javascript)
        self.assertIn("Посмотреть в клубе", javascript)
        ...
        self.assertIn('id="member-shell-header"', index)
        self.assertIn("Закрытый клуб", index)
        self.assertIn("Натальи Ребковец", index)
        self.assertEqual(index.count("data-member-nav="), 4)
        self.assertIn('data-member-nav="member-home"', index)
        self.assertIn('data-member-nav="member-library"', index)
        self.assertIn('data-member-nav="member-schedule"', index)
        self.assertIn('data-member-nav="member-profile"', index)
        self.assertIn('id="member-library-search"', index)
        self.assertIn("renderMemberLibrary", javascript)
        self.assertIn("item.title.toLocaleLowerCase", javascript)
        self.assertIn("memberLibraryItems", javascript)
        self.assertIn("member-preview-mode", javascript)
        self.assertIn("memberShellHeader.hidden = false", javascript)
        self.assertIn("memberShellHeader.hidden = true", javascript)
        ...
        self.assertNotIn("innerHTML", javascript)
        self.assertNotIn("ккал", index.lower())
        self.assertNotIn("калорий", index.lower())
        self.assertNotIn("прогресс", index.lower())
        self.assertNotIn('<img src="http', index.lower())
        self.assertNotIn('url("http', stylesheet.lower())
        member_asset_dir = self.main.MINIAPP_ASSET_DIR / "assets" / "member"
        self.assertTrue((member_asset_dir / "wellness-hero.jpg").is_file())
        self.assertTrue((member_asset_dir / "meditation-calm.jpg").is_file())
        self.assertIn("data:image/jpeg;base64,", stylesheet)
        self.assertIn("member-hero-media", index)
        self.assertIn("member-meditation-media", index)
        self.assertIn('data-member-icon="strength"', index)
        self.assertIn("createElementNS", javascript)
        self.assertIn("hydrateMemberIcons", javascript)
        self.assertIn("loadMemberSchedule", javascript)
        self.assertIn("scheduleImageContainer", javascript)
        self.assertIn("member-schedule-preview", stylesheet)
        self.assertIn('id="member-library-empty-home"', index)
        self.assertIn("member-duration-badge", javascript)
        self.assertIn("body.member-preview-mode", stylesheet)
        self.assertIn(".member-grid", stylesheet)
        self.assertIn(".member-content-card", stylesheet)
        self.assertIn("grid-template-columns: repeat(4, minmax(0, 1fr))", stylesheet)
        ...
        self.assertIn("Дополнительные материалы", index)
        ...
        self.assertIn("subscriptions-toolbar", index)
        self.assertIn("schedule-toolbar", index)
        self.assertIn("system-compact-card", index)
        self.assertIn("width: min(100%, 760px)", stylesheet)
        self.assertIn("grid-template-columns: repeat(2, minmax(0, 1fr))", stylesheet)
        self.assertIn("@media (min-width: 600px)", stylesheet)
        self.assertIn("@media (max-width: 340px)", stylesheet)
        self.assertIn("env(safe-area-inset-bottom)", stylesheet)
        ...
        self.assertIn('.filter-row::after { content: ""; flex: 0 0 6px; }', stylesheet)
        self.assertIn("@media (min-width: 341px) and (max-width: 430px)", stylesheet)
        self.assertIn("font-family: ui-monospace", stylesheet)
        self.assertNotIn("font-size: 36px", stylesheet)
        self.assertNotIn("font-size: 40px", stylesheet)
