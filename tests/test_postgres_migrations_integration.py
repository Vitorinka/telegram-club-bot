...
class PostgresMigrationIntegrationTests(unittest.TestCase):
    ...
    def test_failed_termination_new_active_grace_after_member_lookup_blocks_ban_real_postgres(self):
        run_migrations(self.get_conn)
        main = import_main()
        user_id = 975011
        now = datetime.utcnow()
        old_expiry = now - timedelta(hours=3)
        subscription_id = "sub_failed_975011"
        self.insert_recovery_user(
            user_id, paid=True, expiry_date=old_expiry,
            stripe_customer_id="cus_failed_975011",
        ...
        failed_invoice = mock.Mock(
            id="in_failed_975011", subscription=subscription_id, status="open",
        )
        voided_invoice = mock.Mock(
            id="in_failed_975011", subscription=subscription_id, status="void",
        )
        subscription = mock.Mock(status="past_due", latest_invoice=failed_invoice)
        subscription.cancel.return_value = mock.Mock(status="canceled")

        async def start_new_grace(**_kwargs):
            grace_started = datetime.utcnow()
            conn = self.get_conn()
            with conn.cursor() as cur:
                cur.execute(
                    """
                    UPDATE users
                    SET payment_failed = TRUE, payment_failed_at = %s,
                        grace_period_end = %s
                    WHERE telegram_id = %s
                    """,
                    (grace_started, grace_started + timedelta(hours=48), user_id),
                )
            conn.commit()
            conn.close()
            return SimpleNamespace(status="member")
        ...
        with mock.patch.object(main, "get_db_conn", side_effect=self.get_conn), \
             mock.patch.object(main.stripe.Subscription, "retrieve", return_value=subscription), \
             mock.patch.object(main.stripe.Invoice, "retrieve", side_effect=[failed_invoice, voided_invoice]), \
             mock.patch.object(main.bot, "get_chat_member", mock.AsyncMock(side_effect=start_new_grace)), \
             mock.patch.object(main.bot, "ban_chat_member", mock.AsyncMock()) as ban, \
             mock.patch.object(main.bot, "unban_chat_member", mock.AsyncMock()) as unban:
            result = asyncio.run(main.terminate_failed_subscription(
                user_id, "grace_period_expired"
        ...
        unban.assert_not_awaited()
        self.assertEqual(self.query_one(
            "SELECT status, telegram_removed_at FROM failed_subscription_terminations WHERE telegram_id = %s",
            (user_id,),
        ), ("superseded", None))

    def test_failed_termination_recent_failure_window_after_member_lookup_blocks_ban_real_postgres(self):
        run_migrations(self.get_conn)
        main = import_main()
        user_id = 975012
        now = datetime.utcnow()
        old_expiry = now - timedelta(hours=3)
        subscription_id = "sub_failed_975012"
        self.insert_recovery_user(
            user_id, paid=True, expiry_date=old_expiry,
            stripe_customer_id="cus_failed_975012",
            stripe_subscription_id=subscription_id,
            payment_failed=True,
            payment_failed_at=now - timedelta(hours=49),
            grace_period_end=now - timedelta(hours=1),
        ...
        failed_invoice = mock.Mock(
            id="in_failed_975012", subscription=subscription_id, status="open",
        )
        voided_invoice = mock.Mock(
            id="in_failed_975012", subscription=subscription_id, status="void",
        )
        subscription = mock.Mock(status="past_due", latest_invoice=failed_invoice)
        subscription.cancel.return_value = mock.Mock(status="canceled")

        async def record_recent_failure_without_deadline(**_kwargs):
            conn = self.get_conn()
            with conn.cursor() as cur:
                cur.execute(
                    """
                    UPDATE users
                    SET payment_failed = TRUE, payment_failed_at = NOW(),
                        grace_period_end = NULL
                    WHERE telegram_id = %s
                    """,
                    (user_id,),
        ...
        with mock.patch.object(main, "get_db_conn", side_effect=self.get_conn), \
             mock.patch.object(main.stripe.Subscription, "retrieve", return_value=subscription), \
             mock.patch.object(main.stripe.Invoice, "retrieve", side_effect=[failed_invoice, voided_invoice]), \
             mock.patch.object(main.bot, "get_chat_member", mock.AsyncMock(side_effect=record_recent_failure_without_deadline)), \
             mock.patch.object(main.bot, "ban_chat_member", mock.AsyncMock()) as ban, \
             mock.patch.object(main.bot, "unban_chat_member", mock.AsyncMock()) as unban:
            result = asyncio.run(main.terminate_failed_subscription(
                user_id, "grace_period_expired"
    ...
    def test_failed_termination_pre_ban_fence_typed_decisions_real_postgres(self):
        run_migrations(self.get_conn)
        main = import_main()
        user_id = 97502
        old_expiry = datetime.utcnow() - timedelta(hours=3)
        subscription_id = "sub_failed_97502"
        self.insert_recovery_user(
            user_id, paid=True, expiry_date=old_expiry,
            stripe_subscription_id=subscription_id, auto_renew=True,
        )
        ...
        try:
            with conn.cursor() as cur:
                claim = main.claim_failed_subscription_termination(
                    cur, user_id, subscription_id, "grace_period_expired",
                    "in_failed_97502", "worker-typed", old_expiry,
                )
                operation_id, generation = claim[0], claim[1]
                cur.execute(
                    """
                    UPDATE failed_subscription_terminations
                    SET stripe_cancelled_at = NOW(), collection_stopped_at = NOW()
                    WHERE operation_id = %s
                    """,
                    (operation_id,),
                )
        ...
        def decision():
            with mock.patch.object(main, "get_db_conn", side_effect=self.get_conn):
                return main.failed_termination_pre_ban_fence(
                    operation_id, "worker-typed", generation, user_id,
                    subscription_id, old_expiry,
                )

        self.assertEqual(decision(), "safe_to_remove")

        conn = self.get_conn()
        with conn.cursor() as cur:
            cur.execute(
                "UPDATE users SET expiry_date = %s WHERE telegram_id = %s",
                (datetime.utcnow() + timedelta(days=10), user_id),
            )
        conn.commit()
        conn.close()
        self.assertEqual(decision(), "new_access")

        conn = self.get_conn()
        with conn.cursor() as cur:
            cur.execute(
                "UPDATE users SET expiry_date = %s, stripe_subscription_id = %s WHERE telegram_id = %s",
                (old_expiry, "sub_new_97502", user_id),
            )
        conn.commit()
        conn.close()
        self.assertEqual(decision(), "identity_changed")
        ...
        with conn.cursor() as cur:
            ...
            cur.execute(
                "UPDATE failed_subscription_terminations SET collection_stopped_at = NULL WHERE operation_id = %s",
                (operation_id,),
            )
        conn.commit()
        conn.close()
        self.assertEqual(decision(), "phase_incomplete")

        conn = self.get_conn()
        with conn.cursor() as cur:
            cur.execute(
                "UPDATE failed_subscription_terminations SET owner_id = 'worker-other' WHERE operation_id = %s",
                (operation_id,),
            )
        conn.commit()
        conn.close()
        self.assertEqual(decision(), "ownership_lost")
        self.assertEqual(self.query_one(
            "SELECT status, owner_id FROM failed_subscription_terminations WHERE operation_id = %s",
            (operation_id,),
        ), ("processing", "worker-other"))

    def test_failed_termination_phase_incomplete_before_ban_is_retryable_real_postgres(self):
        run_migrations(self.get_conn)
        main = import_main()
        user_id = 97503
        old_expiry = datetime.utcnow() - timedelta(hours=3)
        subscription_id = "sub_failed_97503"
        self.insert_recovery_user(
            user_id, paid=True, expiry_date=old_expiry,
            stripe_customer_id="cus_failed_97503",
            stripe_subscription_id=subscription_id,
            payment_failed=True,
            payment_failed_at=datetime.utcnow() - timedelta(hours=49),
            grace_period_end=datetime.utcnow() - timedelta(hours=1),
        ...
        failed_invoice = mock.Mock(
            id="in_failed_97503", subscription=subscription_id, status="open",
        )
        voided_invoice = mock.Mock(
            id="in_failed_97503", subscription=subscription_id, status="void",
        )
        subscription = mock.Mock(status="past_due", latest_invoice=failed_invoice)
        subscription.cancel.return_value = mock.Mock(status="canceled")

        async def remove_collection_proof(**_kwargs):
            conn = self.get_conn()
            with conn.cursor() as cur:
                cur.execute(
                    """
                    UPDATE failed_subscription_terminations
                    SET collection_stopped_at = NULL
                    WHERE telegram_id = %s
                    """,
                    (user_id,),
                )
        ...
        with mock.patch.object(main, "get_db_conn", side_effect=self.get_conn), \
             mock.patch.object(main.stripe.Subscription, "retrieve", return_value=subscription), \
             mock.patch.object(main.stripe.Invoice, "retrieve", side_effect=[failed_invoice, voided_invoice]), \
             mock.patch.object(main.bot, "get_chat_member", mock.AsyncMock(side_effect=remove_collection_proof)), \
             mock.patch.object(main.bot, "ban_chat_member", mock.AsyncMock()) as ban:
            result = asyncio.run(main.terminate_failed_subscription(
                user_id, "grace_period_expired"
            ))

        self.assertEqual(result, "phase_incomplete")
        ban.assert_not_awaited()
        self.assertEqual(self.query_one(
            "SELECT status, last_error_category FROM failed_subscription_terminations WHERE telegram_id = %s",
            (user_id,),
        ), ("retryable_failed", "pre_ban_phase_incomplete"))

    def test_ordinary_removal_active_grace_race_stops_at_final_fence_real_postgres(self):
        run_migrations(self.get_conn)
        main = import_main()
        user_id = 97504
        expired = datetime.utcnow() - timedelta(hours=1)
        subscription_id = "sub_race_97504"
        self.insert_recovery_user(
            user_id, paid=True, expiry_date=expired,
            stripe_customer_id="cus_race_97504",
            stripe_subscription_id=subscription_id,
            payment_failed=False, auto_renew=True,
        )

        async def activate_grace_during_member_lookup(**_kwargs):
            now = datetime.utcnow()
            conn = self.get_conn()
            with conn.cursor() as cur:
                cur.execute(
                    ...
                    WHERE telegram_id = %s
                    """,
                    (now, now + timedelta(hours=48), user_id),
                )
            conn.commit()
            conn.close()
            return SimpleNamespace(status="member")
        ...
        with mock.patch.object(main, "get_db_conn", side_effect=self.get_conn), \
             ...
             mock.patch.object(main.bot, "get_chat_member", mock.AsyncMock(side_effect=activate_grace_during_member_lookup)), \
             mock.patch.object(main.bot, "ban_chat_member", mock.AsyncMock()) as ban, \
             mock.patch.object(main, "finalize_subscription_removal_in_db") as finalize:
        ...
        self.assertEqual(self.query_one(
            "SELECT paid, payment_failed, grace_period_end > NOW() FROM users WHERE telegram_id = %s",
            (user_id,),
        ), (True, True, True))
        self.assertNotEqual(self.query_one(
            "SELECT status FROM subscription_removal_events WHERE telegram_id = %s",
            (user_id,),
        )[0], "db_finalized")

    def test_terminal_join_sync_clears_only_stale_auto_renew_guard_real_postgres(self):
        run_migrations(self.get_conn)
        main = import_main()
        user_id = 97505
        expired = datetime.utcnow() - timedelta(hours=1)
        subscription_id = "sub_terminal_97505"
        self.insert_recovery_user(
            user_id, paid=True, expiry_date=expired,
            stripe_subscription_id=subscription_id, auto_renew=True,
        )
        ...
        with mock.patch.object(main, "get_db_conn", side_effect=self.get_conn):
            self.assertFalse(main.unauthorized_group_join_removal_is_still_safe(user_id))
            self.assertTrue(main.sync_terminal_subscription_before_join_removal(
                user_id, subscription_id
            ))
            self.assertTrue(main.unauthorized_group_join_removal_is_still_safe(user_id))

        self.assertEqual(self.query_one(
            "SELECT auto_renew, stripe_subscription_id FROM users WHERE telegram_id = %s",
            (user_id,),
    ...
    def test_reserved_gift_scheduler_skips_live_active_subscription_real_postgres(self):
        ...
        self.assertEqual(result, {"candidates": 0, "applied": 0, "skipped": 0, "retryable": 0})
    ...
    def test_gift_activation_blocked_for_active_auto_renew(self):
        ...
        self.assertIn("активирован автоматически", main.build_gift_reserved_recipient_text(first))
        self.assertEqual(
        ...
        """)
        self.assertEqual(reserved_row[:2], ("reserved", recipient_id))
        self.assertIsNotNone(reserved_row[2])
        self.assertEqual(reserved_row[3:], (None, None, None))
        self.assertEqual(self.query_one("SELECT COUNT(*) FROM access_events WHERE telegram_id = %s", (recipient_id,))[0], 0)
        self.assertEqual(self.query_one(
            "SELECT COUNT(*) FROM gift_access_events WHERE public_reference = %s AND event_type = 'gift_reserved'",
            ("GIFT-0000000000000008",),
        )[0], 1)
        self.assertEqual(
            self.query_one("SELECT COUNT(*) FROM message_delivery_events WHERE delivery_key LIKE %s", ("gift:GIFT-0000000000000008:%",))[0],
            0,
        )
        with mock.patch.object(main, "get_db_conn", side_effect=self.get_conn), mock.patch.object(
            main, "gift_recipient_subscription_state",
            mock.AsyncMock(return_value={"action": "block_active_auto_renew", "subscription_id": "sub_live_auto", "status": "active"}),
        ):
            self.assertEqual(
                asyncio.run(main.apply_reserved_gifts(limit=10)),
                {"candidates": 0, "applied": 0, "skipped": 0, "retryable": 0},
            )
    ...
    def test_reserved_gifts_apply_fifo_one_at_a_time_and_ignore_checkout_expiry_real_postgres(self):
        run_migrations(self.get_conn)
        main = import_main()
        recipient_id = 99340
        conn = self.get_conn()
        ...
        try:
            cur.execute("""
                INSERT INTO users (telegram_id, paid, expiry_date, auto_renew, stripe_subscription_id)
                VALUES (%s, FALSE, NOW() - INTERVAL '1 day', FALSE, NULL)
            """, (recipient_id,))
            gifts = (
                ("00000000-0000-0000-0000-000000099341", "GIFT-0000000000099341", "gift_1m", 30, "3 days"),
                ("00000000-0000-0000-0000-000000099342", "GIFT-0000000000099342", "gift_6m", 180, "2 days"),
            )
            for gift_id, reference, tariff, duration, age in gifts:
                cur.execute("""
                    INSERT INTO gift_access_grants (
                        id, public_reference, purchaser_telegram_id, recipient_telegram_id,
                        recipient_name, sender_name, gift_message, tariff_code, duration_days,
                        status, token_hash, token_version, paid_at, reserved_at,
                        checkout_expires_at, created_at, updated_at
                    ) VALUES (
                        %s, %s, 99349, %s, 'Recipient', 'Sender', '', %s, %s,
                        'reserved', %s, 1, NOW() - (%s::interval), NOW() - (%s::interval),
                        NOW() - INTERVAL '30 days', NOW() - (%s::interval), NOW()
                    )
                """, (
                    gift_id, reference, recipient_id, tariff, duration,
                    main.gift_token_hash_for_reference(reference, 1), age, age, age,
                ))
            conn.commit()
        finally:
            cur.close()
        ...
        safe_state = {"action": "apply", "subscription_id": None, "status": None}
        with mock.patch.object(
            main, "get_db_conn", side_effect=self.get_conn,
        ), mock.patch.object(
            main, "gift_recipient_subscription_state", mock.AsyncMock(return_value=safe_state),
        ):
            first = asyncio.run(main.apply_reserved_gifts(limit=10))
            blocked_second = asyncio.run(main.apply_reserved_gifts(limit=10))

        self.assertEqual(first, {"candidates": 1, "applied": 1, "skipped": 0, "retryable": 0})
        self.assertEqual(blocked_second, {"candidates": 0, "applied": 0, "skipped": 0, "retryable": 0})
        self.assertEqual(
            self.query_one("SELECT status FROM gift_access_grants WHERE public_reference = %s", (gifts[0][1],))[0],
            "redeemed",
        )
        self.assertEqual(
            self.query_one("SELECT status FROM gift_access_grants WHERE public_reference = %s", (gifts[1][1],))[0],
            "reserved",
        )
        first_expiry = self.query_one("SELECT expiry_date FROM users WHERE telegram_id = %s", (recipient_id,))[0]
        self.assertGreater(first_expiry, datetime.utcnow() + timedelta(days=29))

        conn = self.get_conn()
        cur = conn.cursor()
        try:
            cur.execute(
                "UPDATE users SET paid = FALSE, expiry_date = NOW() - INTERVAL '1 second' WHERE telegram_id = %s",
                (recipient_id,),
            )
            conn.commit()
        finally:
        ...
        with mock.patch.object(main, "get_db_conn", side_effect=self.get_conn), mock.patch.object(
            ...
            second = asyncio.run(main.apply_reserved_gifts(limit=10))
            duplicate = asyncio.run(main.apply_reserved_gifts(limit=10))
        self.assertEqual(second, {"candidates": 1, "applied": 1, "skipped": 0, "retryable": 0})
        self.assertEqual(duplicate, {"candidates": 0, "applied": 0, "skipped": 0, "retryable": 0})
        self.assertEqual(self.query_one(
            "SELECT COUNT(*) FROM access_events WHERE telegram_id = %s AND event_type = 'gift_access_redeemed'",
            (recipient_id,),
        )[0], 2)

    def test_reserved_gift_final_fences_new_access_invalid_state_and_recipient_change_real_postgres(self):
        run_migrations(self.get_conn)
        main = import_main()
        recipient_id = 99350

        def insert_reserved(reference, suffix, status="reserved"):
            conn = self.get_conn()
            cur = conn.cursor()
            try:
                cur.execute("""
                    INSERT INTO gift_access_grants (
                        ...
                        recipient_name, sender_name, gift_message, tariff_code, duration_days,
                        status, token_hash, token_version, paid_at, reserved_at
                    ) VALUES (%s, %s, 99359, %s, 'Recipient', 'Sender', '',
                              'gift_1m', 30, %s, %s, 1, NOW(), NOW())
                """, (
                    f"00000000-0000-0000-0000-{suffix:012d}", reference, recipient_id,
                    status, main.gift_token_hash_for_reference(reference, 1),
                ))
                conn.commit()
            finally:
                cur.close()
        ...
        insert_reserved("GIFT-0000000000099351", 99351)

        async def add_new_access(_recipient_id):
            conn = self.get_conn()
            cur = conn.cursor()
            try:
                cur.execute("""
                    INSERT INTO users (telegram_id, paid, expiry_date, auto_renew)
                    VALUES (%s, TRUE, NOW() + INTERVAL '10 days', FALSE)
                    ON CONFLICT (telegram_id) DO UPDATE
                    SET paid = TRUE, expiry_date = NOW() + INTERVAL '10 days'
                """, (recipient_id,))
                conn.commit()
            finally:
                cur.close()
                conn.close()
            return {"action": "apply", "subscription_id": None, "status": None}

        with mock.patch.object(main, "get_db_conn", side_effect=self.get_conn), mock.patch.object(
            main, "gift_recipient_subscription_state", side_effect=add_new_access,
        ):
            protected = asyncio.run(main.apply_reserved_gifts(limit=10))
        self.assertEqual(protected, {"candidates": 1, "applied": 0, "skipped": 1, "retryable": 0})
        self.assertEqual(self.query_one(
            "SELECT status FROM gift_access_grants WHERE public_reference = 'GIFT-0000000000099351'"
        )[0], "reserved")

        conn = self.get_conn()
        cur = conn.cursor()
        try:
            cur.execute("UPDATE users SET paid = FALSE, expiry_date = NULL WHERE telegram_id = %s", (recipient_id,))
            conn.commit()
        finally:
            cur.close()
            conn.close()

        async def change_recipient_after_selection(_recipient_id):
            conn = self.get_conn()
            cur = conn.cursor()
            try:
                cur.execute("""
                    UPDATE gift_access_grants SET recipient_telegram_id = %s
                    WHERE public_reference = 'GIFT-0000000000099351'
                """, (recipient_id + 1,))
                conn.commit()
            finally:
                cur.close()
        ...
        with mock.patch.object(main, "get_db_conn", side_effect=self.get_conn), mock.patch.object(
            main, "gift_recipient_subscription_state", side_effect=change_recipient_after_selection,
        ):
            identity_result = asyncio.run(main.apply_reserved_gifts(limit=10))
        self.assertEqual(identity_result, {"candidates": 1, "applied": 0, "skipped": 1, "retryable": 0})
        self.assertEqual(self.query_one(
            "SELECT paid FROM users WHERE telegram_id = %s", (recipient_id,)
        ), (False,))
        self.assertEqual(self.query_one(
            "SELECT status, recipient_telegram_id FROM gift_access_grants WHERE public_reference = 'GIFT-0000000000099351'"
        ), ("reserved", recipient_id + 1))
        ...
        for offset, state in enumerate(("cancelled", "refunded"), start=2):
            insert_reserved(f"GIFT-000000000009935{offset}", 99350 + offset, status=state)
        with mock.patch.object(main, "get_db_conn", side_effect=self.get_conn), mock.patch.object(
            main, "gift_recipient_subscription_state",
            mock.AsyncMock(return_value={"action": "apply", "subscription_id": None, "status": None}),
        ):
            invalid = asyncio.run(main.apply_reserved_gifts(limit=10))
        self.assertEqual(invalid["candidates"], 0)

    def test_reserved_gift_concurrent_workers_apply_once_real_postgres(self):
        run_migrations(self.get_conn)
        main = import_main()
        recipient_id = 99360
        reference = "GIFT-0000000000099360"
        conn = self.get_conn()
        cur = conn.cursor()
        try:
            cur.execute("""
                INSERT INTO users (telegram_id, paid, expiry_date, auto_renew)
                VALUES (%s, FALSE, NOW() - INTERVAL '1 day', FALSE)
            """, (recipient_id,))
            cur.execute("""
                INSERT INTO gift_access_grants (
                    id, public_reference, purchaser_telegram_id, recipient_telegram_id,
                    ...
                    '00000000-0000-0000-0000-000000099360', %s, 99369, %s,
                    'Recipient', 'Sender', '', 'gift_1m', 30, 'reserved', %s, 1, NOW(), NOW()
                )
            """, (reference, recipient_id, main.gift_token_hash_for_reference(reference, 1)))
            conn.commit()
        finally:
            cur.close()
            conn.close()
        ...
        async def synchronized_state(_recipient_id):
            barrier.wait(timeout=5)
            return {"action": "apply", "subscription_id": None, "status": None}
        ...
        def worker():
            try:
                with mock.patch.object(main, "get_db_conn", side_effect=self.get_conn), mock.patch.object(
                    main, "gift_recipient_subscription_state", side_effect=synchronized_state,
                ):
                    results.append(asyncio.run(main.apply_reserved_gifts(limit=10)))
    ...
    def test_reserved_gift_worker_grace_status_change_and_rollback_retry_real_postgres(self):
        run_migrations(self.get_conn)
        main = import_main()

        def insert_reserved(recipient_id, suffix):
            reference = f"GIFT-{suffix:016d}"
            conn = self.get_conn()
            cur = conn.cursor()
            try:
                cur.execute("""
                    INSERT INTO users (
                        telegram_id, paid, expiry_date, payment_failed,
                        payment_failed_at, grace_period_end, auto_renew
                    ) VALUES (%s, FALSE, NOW() - INTERVAL '1 day', FALSE, NULL, NULL, FALSE)
                ...
                cur.execute("""
                    INSERT INTO gift_access_grants (
                        ...
                        %s, %s, 99499, %s, 'Recipient', 'Sender', '', 'gift_1m', 30,
                        'reserved', %s, 1, NOW(), NOW()
                    )
                """, (
                    f"00000000-0000-0000-0000-{suffix:012d}", reference, recipient_id,
                    main.gift_token_hash_for_reference(reference, 1),
        ...
        grace_recipient = 99470
        grace_reference = insert_reserved(grace_recipient, 99470)
        conn = self.get_conn()
        cur = conn.cursor()
        try:
            cur.execute("""
                ...
                SET payment_failed = TRUE,
                    payment_failed_at = NOW(),
                    grace_period_end = NOW() + INTERVAL '48 hours'
                WHERE telegram_id = %s
            """, (grace_recipient,))
            conn.commit()
        finally:
        ...
        with mock.patch.object(main, "get_db_conn", side_effect=self.get_conn), mock.patch.object(
            ...
            grace_result = asyncio.run(main.apply_reserved_gifts(limit=10))
        self.assertEqual(grace_result, {"candidates": 0, "applied": 0, "skipped": 0, "retryable": 0})
        self.assertEqual(self.query_one(
            "SELECT status FROM gift_access_grants WHERE public_reference = %s", (grace_reference,),
        )[0], "reserved")

        changed_recipient = 99471
        changed_reference = insert_reserved(changed_recipient, 99471)

        async def refund_after_selection(_recipient_id):
            conn = self.get_conn()
            cur = conn.cursor()
            try:
                cur.execute("""
        ...
        with mock.patch.object(main, "get_db_conn", side_effect=self.get_conn), mock.patch.object(
            main, "gift_recipient_subscription_state", side_effect=refund_after_selection,
        ):
            changed_result = asyncio.run(main.apply_reserved_gifts(limit=10))
        self.assertEqual(changed_result, {"candidates": 1, "applied": 0, "skipped": 1, "retryable": 0})
        self.assertEqual(self.query_one(
            "SELECT paid FROM users WHERE telegram_id = %s", (changed_recipient,),
        ), (False,))

        retry_recipient = 99472
        retry_reference = insert_reserved(retry_recipient, 99472)
        original_apply = main.apply_gift_access_in_transaction
        with mock.patch.object(main, "get_db_conn", side_effect=self.get_conn), mock.patch.object(
            main, "gift_recipient_subscription_state", mock.AsyncMock(return_value=safe_state),
        ), mock.patch.object(
            main, "apply_gift_access_in_transaction", side_effect=RuntimeError("simulated_worker_crash"),
        ):
            failed_result = asyncio.run(main.apply_reserved_gifts(limit=10))
        self.assertEqual(failed_result, {"candidates": 1, "applied": 0, "skipped": 0, "retryable": 1})
        self.assertEqual(self.query_one(
            "SELECT status FROM gift_access_grants WHERE public_reference = %s", (retry_reference,),
        )[0], "reserved")
        with mock.patch.object(main, "get_db_conn", side_effect=self.get_conn), mock.patch.object(
            main, "gift_recipient_subscription_state", mock.AsyncMock(return_value=safe_state),
        ), mock.patch.object(main, "apply_gift_access_in_transaction", side_effect=original_apply):
            retry_result = asyncio.run(main.apply_reserved_gifts(limit=10))
        self.assertEqual(retry_result, {"candidates": 1, "applied": 1, "skipped": 0, "retryable": 0})
        self.assertEqual(self.query_one(
            "SELECT COUNT(*) FROM access_events WHERE telegram_id = %s AND event_type = 'gift_access_redeemed'",
            (retry_recipient,),
        )[0], 1)
    ...
    def test_miniapp_manual_access_is_atomic_fenced_and_idempotent_real_postgres(self):
        ...
        with mock.patch.object(main, "get_db_conn", side_effect=self.get_conn):
            exact_preview = json.loads(asyncio.run(main.miniapp_admin_access_preview(
                JsonAdminRequest(1, {"telegram_id": "88304"}, {"duration": "7d"}))).text)
        exact_expected = datetime.fromisoformat(exact_preview["proposed_expiry"])
        exact_action = self.get_conn()
        try:
            with exact_action.cursor() as cur:
                ...
                exact_payload["preview_expires_at"] = (
                    delayed_created + main.MANUAL_ACCESS_PREVIEW_TTL
                ).isoformat()
                cur.execute("UPDATE admin_action_requests SET payload_json = %s WHERE action_id = %s",
                            (json.dumps(exact_payload, sort_keys=True), exact_preview["action_id"]))
            exact_action.commit()
        finally:
            exact_action.close()
        with mock.patch.object(main, "get_db_conn", side_effect=self.get_conn):
            exact_confirm = asyncio.run(main.miniapp_admin_access_confirm(
                JsonAdminRequest(1, {"telegram_id": "88304"}, {"action_id": exact_preview["action_id"]})))
        self.assertEqual(exact_confirm.status, 200)
        self.assertEqual(datetime.fromisoformat(json.loads(exact_confirm.text)["expiry_date"]), exact_expected)
        exact_verify = self.get_conn()
        try:
            with exact_verify.cursor() as cur:
                cur.execute("SELECT expiry_date FROM users WHERE telegram_id = 88304")
                self.assertEqual(cur.fetchone()[0], exact_expected)
        finally:
            exact_verify.close()

        with mock.patch.object(main, "get_db_conn", side_effect=self.get_conn):
            expired_preview = json.loads(asyncio.run(main.miniapp_admin_access_preview(
                JsonAdminRequest(1, {"telegram_id": "88301"}, {"duration": "90d"}))).text)
        expired_action = self.get_conn()
        try:
            with expired_action.cursor() as cur:
                cur.execute("SELECT payload_json FROM admin_action_requests WHERE action_id = %s",
                            (expired_preview["action_id"],))
                expired_payload = json.loads(cur.fetchone()[0])
                old_preview_time = datetime.utcnow() - timedelta(minutes=11)
                expired_payload["preview_created_at"] = old_preview_time.isoformat()
                expired_payload["preview_expires_at"] = (
                    old_preview_time + main.MANUAL_ACCESS_PREVIEW_TTL
                ).isoformat()
                cur.execute("UPDATE admin_action_requests SET payload_json = %s WHERE action_id = %s",
                            (json.dumps(expired_payload, sort_keys=True), expired_preview["action_id"]))
                # Even if time-sensitive access state crosses its deadline, TTL rejection wins
                # before the unchanged state HMAC can authorize a stale preview.
                cur.execute("UPDATE users SET expiry_date = expiry_date WHERE telegram_id = 88301")
            expired_action.commit()
        finally:
            expired_action.close()
        with mock.patch.object(main, "get_db_conn", side_effect=self.get_conn):
            expired_confirm = asyncio.run(main.miniapp_admin_access_confirm(
                JsonAdminRequest(1, {"telegram_id": "88301"}, {"action_id": expired_preview["action_id"]})))
            expired_duplicate = asyncio.run(main.miniapp_admin_access_confirm(
                JsonAdminRequest(1, {"telegram_id": "88301"}, {"action_id": expired_preview["action_id"]})))
        self.assertEqual(expired_confirm.status, 409)
        self.assertEqual(json.loads(expired_confirm.text)["error"], "manual_access_preview_expired")
        self.assertEqual(json.loads(expired_duplicate.text)["failure_category"],
                         "manual_access_preview_expired")
        expired_verify = self.get_conn()
        try:
            with expired_verify.cursor() as cur:
                cur.execute("SELECT expiry_date FROM users WHERE telegram_id = 88301")
                self.assertEqual(cur.fetchone()[0], expired)
                cur.execute("SELECT status FROM admin_action_requests WHERE action_id = %s",
                            (expired_preview["action_id"],))
                self.assertEqual(cur.fetchone()[0], "failed")
        finally:
            expired_verify.close()

        with mock.patch.object(main, "get_db_conn", side_effect=self.get_conn):
            short_active_preview = json.loads(asyncio.run(main.miniapp_admin_access_preview(
        ...
        with mock.patch.object(main, "get_db_conn", side_effect=self.get_conn), \
            ...
            short_grace_confirm = asyncio.run(main.miniapp_admin_access_confirm(
                JsonAdminRequest(1, {"telegram_id": "88306"},
                                 {"action_id": short_grace_preview["action_id"]})))
        self.assertEqual(json.loads(short_active_confirm.text)["error"],
                         "manual_access_preview_expired")
        self.assertEqual(json.loads(short_grace_confirm.text)["error"],
                         "manual_access_preview_expired")
        passage_verify = self.get_conn()
        try:
            with passage_verify.cursor() as cur:
                cur.execute("SELECT telegram_id, expiry_date, payment_failed, grace_period_end "
                            "FROM users WHERE telegram_id IN (88305, 88306) ORDER BY telegram_id")
        ...
        try:
            with verify.cursor() as cur:
                ...
                self.assertEqual(parsed_payload["proposed_expiry"], grace_preview["proposed_expiry"])
                self.assertIn("preview_created_at", parsed_payload)
