...
async def stripe_webhook(request):
    ...
    if event['type'] == 'checkout.session.completed':
        ...
        logging.info(
            ...
            f"customer_id={stripe_object_id(stripe_value(session, 'customer'))}, "
            f"customer_email={stripe_value(session, 'customer_details', 'email') or stripe_value(session, 'customer_email')}"
        )
        if not user_id:
            logging.error(
        ...
        try:
            ...
            await log_access_event(
                ...
                stripe_subscription_id=sub_id,
                notes=f"days={days_to_add}; customer_id={customer_id or 'нет'}; customer_email={customer_email or 'нет'}"
            )
    ...
    # ---------- 2. УСПЕШНОЕ АВТОПРОДЛЕНИЕ (invoice.payment_succeeded) ----------
        ...
        try:
            ...
            if not row:
                ...
                if linked_telegram_id:
                    cur.execute("""
                        ...
                        WHERE users.telegram_id = target.telegram_id
                        RETURNING users.telegram_id, target.old_expiry, target.was_payment_failed
                    """, (linked_telegram_id, new_expiry, new_expiry, sub_id, customer_id))
                    row = cur.fetchone()
                    if row:
                        old_expiry = row[1]
                        was_payment_failed = row[2]
            ...
            needs_rejoin_invite = (old_expiry is None) or (old_expiry < datetime.utcnow())
            if needs_rejoin_invite:
                await send_rejoin_invite_after_payment(
                    telegram_id,
                    new_expiry,
            ...
            try:
                ...
                logging.info(
                    "AUTO_RENEW_NOTICE_SENT: telegram_id=%s, invoice_id=%s, new_expiry=%s",
                    telegram_id,
                    invoice_id,
                    new_expiry,
                )
            ...
            if status in ("past_due", "unpaid"):
                ...
                cur.execute("""
                    ...
                    SET paid = TRUE,
                        ...
                        payment_failed = FALSE,
                        payment_failed_at = NULL,
                        last_payment_succeeded_at = COALESCE(last_payment_succeeded_at, NOW()),
                        grace_period_end = NULL,
                        reminder_sent = FALSE,
                        auto_renew = %s,
                        stripe_customer_id = COALESCE(%s, stripe_customer_id)
                    WHERE stripe_subscription_id = %s
                    RETURNING telegram_id, paid, expiry_date
                """, (subscription_expiry, subscription_expiry, not cancel_at_period_end, customer_id, sub_id))
                row = cur.fetchone()
                if row:
                    upsert_stripe_link(
                        cur,
                    ...
                    logging.info(
                        "SUBSCRIPTION_ACTIVE_STATE_RESTORED: event_id=%s, event.type=%s, telegram_id=%s, "
                        "customer_id=%s, subscription_id=%s, status=%s, cancel_at_period_end=%s, "
                        "paid=%s, expiry_date=%s, current_period_end=%s",
                        event_id,
                        event_type,
                        row[0],
                        customer_id,
